...
//        fun calc(graph: List<Edge>, source: Int) {
//            val n = graph.size
...
//        }
        ...
        fun calc(graph: List<Edge>, n: Int, source: Int, destination: Int, print: Boolean) {
            val p = IntArray(n) { -1 }
            ...
            while (true) {
                ...
                for (j in graph.indices) {
                    if (d[graph[j].from] < INF) {
                        if (d[graph[j].to] > d[graph[j].from] + graph[j].weight) {
                            d[graph[j].to] = d[graph[j].from] + graph[j].weight
